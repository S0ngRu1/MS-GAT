...
# 读取 CSV 文件
df = pd.read_csv("datasets/Weibo17/test.csv") 
result = []
for index, row in df.iterrows():
    try:
        print(f"正在处理index: {index}")
        image_path = row['image_name']
        image_file = f'datasets/Weibo17/new_images/{image_path}'
...
# 将所有结果保存为 JSON 文件
with open('datasets/Weibo17/ocr_data_test.json', 'w', encoding='utf-8') as f:
