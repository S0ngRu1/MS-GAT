...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', nargs='+', default=42, help='List of random seeds')
    ...
    args.model_save_path = os.path.join(args.model_save_dir, f'{args.model_name}.pth')
    args.best_model_save_path = os.path.join(args.model_save_dir, f'{args.model_name}-best.pth')
    
    if not os.path.exists(args.logs_dir):
        os.makedirs(args.logs_dir)
    logger.add(os.path.join(args.logs_dir, f'{args.model_name}.log'))
    args_dict = vars(args)
    logger.info("超参数配置：")
    logger.info(pprint.pformat(args_dict, indent=2))
